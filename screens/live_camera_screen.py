...
class LiveCameraScreen(Screen):
    ...
    def on_enter(self, *args):
        MDApp.get_running_app().root.get_screen("home").set_current_screen('live_camera')
        ...
        top_app_bar = MDTopAppBar(
            title="Live Camera",
            left_action_items=[['arrow-left', lambda x: setattr(MDApp.get_running_app().root,'current', MDApp.get_running_app().root.get_screen("home").goto_prev_screen())]],
            elevation=3
        )

        open_camera_button = MDRoundFlatButton(
            text="Open Camera",
            pos_hint={"center_x": 0.5, "center_y": 0.5},
            on_release=MDApp.get_running_app().open_camera
        )

        self.ids.live_camera_image.add_widget(open_camera_button)

        self.add_widget(top_app_bar)
        self.ids.live_camera_image.add_widget(self.image)
